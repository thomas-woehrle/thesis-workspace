...
def get_num_topk_correct_preds(
    batch_logits: torch.Tensor, batch_target: torch.Tensor, topk: int
):
    _, batch_topk_indices = torch.topk(batch_logits, topk, dim=-1)
    batch_topk_pred_correct = (batch_topk_indices == batch_target.unsqueeze(-1)).sum(
        dim=-1
    ) > 0
...
class Evaluator1:
    ...
    def evaluate_epoch(self, epoch: int):
        ...
        self.logger.log(
            {
                "val/top1_accuracy": top1_correct_preds.sum().item()
                / total_preds.sum().item()
            },
            epoch,
        )
        self.logger.log(
            {
                "val/top5_accuracy": top5_correct_preds.sum().item()
                / total_preds.sum().item()
            },
            epoch,
        )
